...
* **ルアーアクションの一般化**: ルアー使用回数（0〜3回）において発生しうる「発見」「型確定」「なし」の分岐を整理し、全32パターンの発生論理を数式化。
* **隠し魚出現ロジックの解明**: 未発見状態では重み 0 となり、「発見」が発生した瞬間から固有のヒット率が適用される特殊な抽選ロジックを定義。

## ステップ 1：手動設定シミュレータの構築（現在完了）
...
* **期待待機秒数 () の導入**: ターゲット1匹を得るために必要な「正味の拘束時間」を算出する独自指標を確立。
* **計算プロセスの透明化**: デバッグ・解析ビューを実装し、重み補正や数式の実行トレースを可視化することで、計算結果の正当性を検証可能にした。
