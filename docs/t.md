...
- **型 (Jaws)**: `small jaws` / `large jaws` の分類。
...
- **餌 (Bait)** / **時間・天気条件 (Time / Weather)** / **発見の有無 (Discovery status)** / **直感の有無 (Intuition status)**

#### 出現の判定（重み 0 の定義）
特定の条件（特定の天気、あるいは発見・直感等）を満たしていない魚種は、その時点の重みが $0$ となり、抽選テーブルから物理的に除外される。

#### 効果の有効期間と揮発性（リセットタイミング）
...
---
#### 時間制限の定義
撒きき餌、ルアー、トレードリリースには、**バフとしての時間制限は存在しない**。一度付与された効果は、上記の終了条件を満たすまで永続する。

---
#### 隠し魚の重み規則
「発見」のアクションによってのみ出現する隠し魚の重みおよびヒット率は、通常魚とは異なる特殊なルール（第6章参照）に基づいて制御される。
...
- **発見 (Discovery) 成功時の挙動**: 隠し魚が出現された状態。隠し魚のヒット率は直接指定値 $P_{DiscHit}$ が適用され、残りの確率を通常魚同士で分配する。
...
| **撒き餌** | キャスト前 | `C_CHUM` | ヒット時間を 0.6倍にする。**（リセット条件は4.4参照）** |
| **ルアー** | 待機中 | `M_Nx` / `DISC` / `GUAR` | 重み補正・発見・確定判定。**（リセット条件は4.4参照）** |
| **トレードリリース** | キャスト前 | `WEIGHT = 0` | 特定魚種を除外。**釣り上げ時のみ消失（4.4参照）** |
| **フッキング** | アタリ発生時 | `D_HOOK` | 魚を釣り上げ、サイクルを終了する。 |
| **竿上げ (Rod Lift)** | アタリ発生時 | `D_REST` | 釣り上げを中断し、サイクルを終了する。 |
...
5. **終了フェーズ (Finish Phase) ※条件分岐**
   - **パターンA：釣り上げ (Catch)**
     - `D_HOOK` を消費。終了後、**「4.4」に基づき全ての状態（TR含む）をリセット**する。
   - **パターンB：竿上げ (Rod Lift)**
     - `D_REST` を消費。終了後、**「4.4」に基づきTR以外の状態をリセット**する。
### 5.2 時間算出数式
- **実効待機時間 ($T_{Final}$)**:
...
**名称生成の例:**
- L3で1回目に発見、3回目に型確定： `L3: 発見1, 型確定(発見済)3`
