# 漁師論理エンジン：ゲームドメイン定義書 (Game Domain Spec) v3.3.0
...
- **隠し魚 (Hidden)**: 通常の抽選テーブルには含まれず、ルアーアクションによる「発見」状態でのみ出現する特殊な魚種。
- **データの正規化**: 
...
特定の条件（特定の天気、あるいは発見・直感等）を満たしていない魚種は、その時点の重みが $0$ となり、抽選テーブルから物理的に除外される。「隠し魚（Hidden）は、未発見状態においては重み $w$ を一貫して $0$ とし、2.1 の基本ヒット率算出の分母（全出現候補の重みの総和）には含めないものとする
...
- `WEIGHT` ($w_i$): `基礎重み・基礎待機時間.csv` に定義される各魚種固有の数値。
- **トレードリリース (Surface Slap)**: 使用時、対象魚の基礎重みを $0$ とする。`マスタ - 魚種.csv` の「トレード可否」列が 0 の魚種には、トレードリリースを使用することはできない（UI制御）。
...
5. **終了フェーズ (Finish Phase) ※条件分岐**　終了後、各状態のリセット条件は「4.4」の表に従う。
   ...
   - **パターンB：竿上げ (Rod Lift)**
     - `D_REST` を消費。
### 5.2 時間算出数式
- **実効待機時間 ($T_{Final}$) の期待値算出 (v3.2.1 改訂)**:
  ルアー拘束時間 $L$ と、魚種の基礎待機時間分布 $X \sim U[Min, Max]$ の関係 $W = \max(X, L)$ に基づき、以下の3ケースで算出する。
  1. **Case 1 ($L \le Min$)**: $L$の影響なし。標準的な平均値 $\frac{Min + Max}{2}$
  2. **Case 2 ($L \ge Max$)**: 常に$L$に拘束される。期待値は $L$
  3. **Case 3 ($Min < L < Max$)**: 積分計算による厳密な期待値。
     $$E[W] = \frac{L(L - Min) + \frac{Max^2 - L^2}{2}}{Max - Min}$$

  ※撒き餌使用時は $Min, Max$ に $C_{Chum}$ を乗じた値を使用する。
...
**名確率定数の決定ロジック:**
- 重みバランスの変化が発見率・型確定率・隠しヒット率に与える計算式は未定であるため、現時点では 発見・型確定・隠しヒット率.csv に定義された実測値を正本として採用する。
---

## 7. ヒット率算出および動作時間詳細仕様
マトリクスの各パターン着地時、以下のロジックで最終的な魚種決定を行う。

### 7.1 隠し魚の直接ヒット率（9パターンの適用）
発見済み状態における隠し魚のヒット率は、以下の3要素の組み合わせに基づき、CSVの専用カラム（9種）から一意の値を参照する。
1. **発見回 ($i$)**: 何回目のアクションで発見したか。
2. **終了回 ($n$)**: 何回目のアクションで釣りを終了したか。
3. **型確定状態**: 終了時点で型確定演出が発生しているか（あり/なし）。
4. **ルアー適合性ゲート:**: 隠し魚の「発見」判定は、使用中のルアーの型（アンビシャス=大型 / モデスト=小型）が隠し魚の「型」と一致している場合にのみ実行される。不一致の場合、発見率は一貫して 0% となる。

### 7.2 通常魚の相対ヒット率
隠し魚当選以外の残確率（$1 - P_{Hidden}$）を、以下の計算式で通常魚に分配する。
$$P_i = \frac{w_i \times M}{\sum (w_k \times M)} \times (1 - P_{Hidden})$$
- $w$: `基礎重み・基礎待機時間.csv` の「基礎重み」列。トレードリリース対象は $w=0$。
...
## 8. CSVデータ連携定義
### 8.1 魚種マスタ (`マスタ - 魚種.csv`)
- **列定義**: `魚種`, `型`, `演出`, `釣り上げ動作時間`, `隠し判定` (0:通常 / 1:隠し), `トレード可否` (0:不可 / 1:可)

### 8.2 釣り場定義 (`マスタ - 釣り場定義.csv`)
- **列定義**: `データ種別`, `釣り場`, `データ`
- **データ種別リスト**: `{天気 / 時間, 餌, 魚種}`

### 8.3 基礎重み・基礎待機時間マスタ (`マスタ - 基礎重み・基礎待機時間.csv`)
- **列定義**: `釣り場`, `天気 / 時間`, `餌`, `魚種`, `基礎重み`, `基礎待機時間min`, `基礎待機時間max`

### 8.4 発見・型確定・隠しヒット率マスタ (`マスタ - 発見・型確定・隠しヒット率.csv`)
- **列定義**:
